//
//  ClaimAthleteLandingViewController.swift
//  MPScoreboard3
//
...
class ClaimAthleteLandingViewController: UIViewController, OnboardingAlertViewDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        backgroundImageView.frame = CGRect(x: 0, y: 0, width: kDeviceWidth, height: kDeviceHeight)
        ...
        titleLabel.frame = CGRect(x: 22, y: kDeviceHeight - 233 - CGFloat(SharedData.bottomSafeAreaHeight), width: kDeviceWidth - 44, height: 31)
        subtitleLabel.frame = CGRect(x: 22, y: kDeviceHeight - 200 - CGFloat(SharedData.bottomSafeAreaHeight), width: kDeviceWidth - 44, height: 90)
        
        claimProfileButton.layer.cornerRadius = claimProfileButton.frame.size.height / 2.0
        claimProfileButton.clipsToBounds = true
        
        // Use the shorter background for medium aspect devices
        if (SharedData.deviceAspectRatio as! AspectRatio == .medium)
        {
            backgroundImageView.image = UIImage(named: "ClaimAthleteBackground-667")
            
            let scaleFactor = 0.85
            let height = 575.0 * scaleFactor
            let width = kDeviceWidth * scaleFactor
            athleteImageView.frame = CGRect(x: (kDeviceWidth - width) / 2.0, y: -20, width: width, height: height)
        }
        else
        {
            // Resize the athlete image for larger devices
            let scaleFactor = kDeviceWidth / 375.0
            let height = 575.0 * scaleFactor
            athleteImageView.frame = CGRect(x: 0, y: 0, width: kDeviceWidth, height: height)
        }
        
        if (self.userIsAthlete == true)
        {
            titleLabel.text = "Claim your MaxPreps profile"
            ...
            athleteImageView.image = UIImage(named: "ClaimAthleteImage")
        }
        else
        {
            ...
            athleteImageView.image = UIImage(named: "ClaimAthleteParentImage")
        }
        ...
        */
        ...
        TrackingManager.trackState(featureName: "claim-home", trackingGuid: trackingGuid, cData: kEmptyTrackingContextData)
    }
