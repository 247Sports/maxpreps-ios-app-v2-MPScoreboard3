//
//  NewScoreboardContestListViewController.swift
//  MPScoreboard3
//
//  Created by David Smith on 8/29/23.
//
...
class NewScoreboardContestListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, NewModalWebViewControllerDelegate, DTBAdCallback, GADBannerViewDelegate, UISearchTextFieldDelegate
{
    ...
    var selectedScoreboard: Dictionary<String,String> = [:]
    
    private var leftShadow : UIImageView!
    private var rightShadow : UIImageView!
    
    private var allContestsArray: Array<Dictionary<String,Any>> = []
    private var availableDatesArray: Array<Date> = []
    private var contestResultsArray: Array<Dictionary<String,Any>> = []
    private var filteredContestsResultsArray: Array<Dictionary<String,Any>> = []
    ...
    private var tableViewStartingHeight = 0.0
    ...
    // MARK: - Filter Contests
    
    private func filterContests()
    {
        if (searchTextField.text!.count > 0)
        {
            filteredContestsResultsArray.removeAll()
            ...
            for contest in contestResultsArray
            {
                ...
                if (teamAName.lowercased().count >= searchTextField.text!.count)
                {
                    if (teamAName.lowercased().starts(with: searchTextField.text!.lowercased()))
                    {
                        matchFound = true
                    }
                }
                if (teamBName.lowercased().count >= searchTextField.text!.count)
                {
                    if (teamBName.lowercased().starts(with: searchTextField.text!.lowercased()))
                    {
                        matchFound = true
                    }
                }
                ...
                if (matchFound == true)
                {
                    filteredContestsResultsArray.append(contest)
                }
            }
        }
        
        print("Filtered Contests: " + String(filteredContestsResultsArray.count))
    ...
    private func getContestResults()
    {
        ...
        var includeRankings = false
        let scoreboardName = selectedScoreboard[kScoreboardDefaultNameKey]
        if (scoreboardName == "national")
        {
            includeRankings = true
        }
        
        NewFeeds.getScoreboardContestResults(contests: contests, includeRankings: includeRankings) { results, error in
    ...
    {
        var activeArray = contestResultsArray
        
        if (searchTextField.text!.count > 0)
        {
            activeArray = filteredContestsResultsArray
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        if (activeArray.count == 0)
        {
            ...
            if (searchTextField.text!.count > 0)
            {
                cell?.textLabel?.text = String(format: "No search results for \"%@\"",  searchTextField.text!)
            }
            else
            {
                cell?.textLabel?.text = ""
        ...
        else
        {
            ...
            if (activeArray.count > indexPath.row)
            {
                ...
                let contest = activeArray[indexPath.row]
                ...
                // Get the rankings to determine which cell to use
                ...
                if (contestState == 5)
                {
                    if ((teamANationalRankValue != -1) || (teamBNationalRankValue != -1))
                    {
                        // Use the ScoreboardContestNoScoreRankingTableViewCell
                        var cell = tableView.dequeueReusableCell(withIdentifier: "ScoreboardContestNoScoreRankingTableViewCell") as? ScoreboardContestNoScoreRankingTableViewCell
                        
                        if (cell == nil)
                        {
                            let nib = Bundle.main.loadNibNamed("ScoreboardContestNoScoreRankingTableViewCell", owner: self, options: nil)
                            cell = nib![0] as? ScoreboardContestNoScoreRankingTableViewCell
                        }
                ...
                else
                {
                    if ((teamANationalRankValue != -1) || (teamBNationalRankValue != -1))
                    {
                        // Use the ScoreboardContestRankingTableViewCell if one team has rankings
                        var cell = tableView.dequeueReusableCell(withIdentifier: "ScoreboardContestRankingTableViewCell") as? ScoreboardContestRankingTableViewCell
                        
                        if (cell == nil)
                        {
                            let nib = Bundle.main.loadNibNamed("ScoreboardContestRankingTableViewCell", owner: self, options: nil)
                            cell = nib![0] as? ScoreboardContestRankingTableViewCell
                        }
                    ...
                    {
                        // Use the ScoreboardContestTableViewCell if no rankings
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        if (hasContestPage == true)
        {
            ...
            {
                self.showModalWebViewController(urlString: urlString, showShareButton: true, contestId: "", sport: "", contestDate: date, duplicateNotification: false)
            }
        }
        else
        {
    ...
    @objc private func keyboardDidShow(notification: Notification)
    {
        if let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue
        {
            ...
            // Shrink the contestTableView height
            contestTableView.frame.size = CGSize(width: kDeviceWidth, height: tableViewStartingHeight - keyboardSize.size.height + 40)
        }
    }
    ...
    @objc private func keyboardDidHide(notification: Notification)
    {
        ...
        // Reset the contestTableView size
        contestTableView.frame.size = CGSize(width: kDeviceWidth, height: tableViewStartingHeight)
    }
    ...
    override func viewDidLoad()
    {
        ...
        searchContainerView.frame = CGRect(x: 0, y: fakeStatusBar.frame.size.height + navView.frame.size.height, width: kDeviceWidth, height: searchContainerView.frame.size.height)
        contestTableView.frame = CGRect(x: 0, y: fakeStatusBar.frame.size.height + navView.frame.size.height + searchContainerView.frame.size.height, width: kDeviceWidth, height: kDeviceHeight - fakeStatusBar.frame.size.height - navView.frame.size.height - searchContainerView.frame.size.height - CGFloat(SharedData.bottomSafeAreaHeight))
        
        tableViewStartingHeight = contestTableView.frame.size.height
