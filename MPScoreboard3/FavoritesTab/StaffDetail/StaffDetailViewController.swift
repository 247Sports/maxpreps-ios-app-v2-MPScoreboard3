...
class StaffDetailViewController: UIViewController, UIScrollViewDelegate, UITableViewDelegate, UITableViewDataSource, StaffHistoryTableViewCellDelegate, StaffInfoSocialTableViewCellDelegate, DTBAdCallback, GADBannerViewDelegate
{
    ...
    private var tableViewInitialHeight = 0
    private var tableHasCareerSection = false
        ...
        RosterFeeds.getStaffDetails(staffUserId: self.selectedStaff!.userId) { result, error in
            ...
            if (error == nil)
            {
                ...
                if (self.profileData.count > 0)
                {
                    ...
                    if (myTeams.count > 0)
                    {
                        self.tableHasCareerSection = true
                    }
                    /*
                    else
                    {
    ...
    // MARK: - StaffInfoSocialDelegate Methods
    ...
    {
        if (self.tableHasCareerSection == true)
        {
            return 3
        }
        else
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat
    {
        ...
        {
            // Only show the first section if the userId = coachUserId
            let userId = kUserDefaults.string(forKey: kUserIdKey)
            if (self.selectedStaff!.userId == userId)
            {
                return 64.0
            }
            else
        ...
        else if (indexPath.section == 1)
        {
            ...
            {
                // Hide the cell if there are no social items
                var facebookUrl = ""
                var twitterHandle = ""
                ...
                {
                    facebookUrl = self.profileData["facebookUrl"] as! String
                }
                
                if ((twitterHandle == "") && (facebookUrl == ""))
                {
                    return 0.0
                }
                else
                {
                    return 100.0 // StaffInfoSocialTableViewCell
                }
            }
            else
            {
                return 80.0 // StaffInfoTeamsTableViewCell
            }
        }
        else
        {
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat
    {
        if (section == 0)
        {
            return staffContainerView.frame.size.height
        }
        else if (section == 1)
        {
            return 64.0
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView?
    {
        if (section == 0)
        {
            // The top header is the same size as the staffContainerView
        ...
        else if (section == 1)
        {
            ...
            let headerNib = Bundle.main.loadNibNamed("StaffInfoHeaderViewCell", owner: self, options: nil)
            let headerView = headerNib![0] as? StaffInfoHeaderViewCell
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        if (indexPath.section == 0)
        {
            var cell = tableView.dequeueReusableCell(withIdentifier: "StaffHeaderMessageTableViewCell") as? StaffHeaderMessageTableViewCell
            
            if (cell == nil)
            {
                let nib = Bundle.main.loadNibNamed("StaffHeaderMessageTableViewCell", owner: self, options: nil)
                cell = nib![0] as? StaffHeaderMessageTableViewCell
            }
        ...
        else if (indexPath.section == 1)
        {
            if (indexPath.row == 0)
            {
                var cell = tableView.dequeueReusableCell(withIdentifier: "StaffInfoBioTableViewCell") as? StaffInfoBioTableViewCell
                
                if (cell == nil)
                {
                    let nib = Bundle.main.loadNibNamed("StaffInfoBioTableViewCell", owner: self, options: nil)
                    cell = nib![0] as? StaffInfoBioTableViewCell
                }
            ...
            {
                var cell = tableView.dequeueReusableCell(withIdentifier: "StaffInfoSocialTableViewCell") as? StaffInfoSocialTableViewCell
                
                if (cell == nil)
                {
                    let nib = Bundle.main.loadNibNamed("StaffInfoSocialTableViewCell", owner: self, options: nil)
                    cell = nib![0] as? StaffInfoSocialTableViewCell
                }
                
                cell?.selectionStyle = .none
                cell?.delegate = self
                cell?.loadData(self.profileData)
                
                return cell!
            }
            else
            {
                var cell = tableView.dequeueReusableCell(withIdentifier: "StaffInfoTeamsTableViewCell") as? StaffInfoTeamsTableViewCell
                
                if (cell == nil)
                {
                    let nib = Bundle.main.loadNibNamed("StaffInfoTeamsTableViewCell", owner: self, options: nil)
                    cell = nib![0] as? StaffInfoTeamsTableViewCell
                }
        ...
        {            
            // Career History Cell
            var cell = tableView.dequeueReusableCell(withIdentifier: "StaffHistoryTableViewCell") as? StaffHistoryTableViewCell
            
            if (cell == nil)
            {
                let nib = Bundle.main.loadNibNamed("StaffHistoryTableViewCell", owner: self, options: nil)
                cell = nib![0] as? StaffHistoryTableViewCell
            }
    ...
    // MARK: - StaffHistoryTableViewCell Delegate
    ...
    @objc private func twitterButtonTouched(_ sender: UIButton)
    {
        ...
        self.showWebBrowser(urlString: urlString, title: "X", showShareButton: true, showLoading: true, whiteHeader: false)
    }
        ...
        */
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0)
        {
            ...
            { (finished) in
                ...
                self.staffTableView.frame.size = CGSize(width: CGFloat(kDeviceWidth), height: CGFloat(self.tableViewInitialHeight) - 60.0)
            }
        }
    }
    ...
    private func clearBannerAd()
    {
        ...
        staffTableView.frame.size = CGSize(width: CGFloat(kDeviceWidth), height: CGFloat(tableViewInitialHeight))
    }
    ...
    override func viewDidLoad()
    {
        ...
        tableViewInitialHeight = Int(kDeviceHeight) - Int(fakeStatusBar.frame.size.height) - Int(navView.frame.size.height) - SharedData.bottomSafeAreaHeight - bottomTabBarPad + 12
         
        staffTableView.frame = CGRect(x: 0, y: fakeStatusBar.frame.size.height + navView.frame.size.height - 12, width: CGFloat(kDeviceWidth), height: CGFloat(tableViewInitialHeight))
