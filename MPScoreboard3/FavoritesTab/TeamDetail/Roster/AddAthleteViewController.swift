...
protocol AddAthleteViewControllerDelegate: AnyObject
{
    func addAthleteSaveButtonTouched()
    func addAthleteCancelButtonTouched()
}

class AddAthleteViewController: UIViewController, UITextFieldDelegate, IQActionSheetPickerViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
    weak var delegate: AddAthleteViewControllerDelegate?
    ...
    private func saveAthlete(single: Bool)
    {
        ...
        {
            // There are no positions or weightClass in golf or tennis
        ...
        RosterFeeds.addSecureAthlete(schoolId: selectedTeam!.schoolId, ssid: self.ssid! , firstName: firstNameTextField.text!, lastName: lastNameTextField.text!, classYear: gradeString, jersey: jerseyTextField.text!, heightFeet: heightFeetString, heightInches: heightInchesString, weight: weightString, weightClass: weightClassString, positions: positionsString, gender: genderTextField.text!, isCaptain: isCaptain, bio: "") { result, error in
            ...
            if (error == nil)
            {
                print("Add Athlete Success")
                
                self.athleteAdded = true
                        ...
                        RosterFeeds.addAthletePhoto(schoolId: self.selectedTeam!.schoolId, ssid: self.ssid!, athleteId: athleteId, imageData: data) { result, error in
                            ...
                            if (error == nil)
                            {
                                ...
                                OverlayView.showPopupOverlay(withMessage: "Athlete Added")
                                {
                                    if (single == true)
                                    {
                                        self.delegate?.addAthleteSaveButtonTouched()
                                    }
                                    else
                                    {
            ...
            {
                print("Add Athlete Failed")
                
                MiscHelper.showAlert(in: self, withActionNames: ["OK"], title: "We're Sorry", message: "Something went wrong when trying to save this athlete.", lastItemCancelType: false) { tag in
    ...
    {
        self.delegate?.addAthleteCancelButtonTouched()
    }
