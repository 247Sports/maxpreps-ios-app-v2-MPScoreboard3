//
//  EditExtracurricularsViewController.swift
//  MPScoreboard3
//
...
class EditExtracurricularsViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate
{
    ...
    @IBAction func saveButtonTouched()
    {
        ...
        let extracurricularId = dataObj["careerExtracurricularId"] as? String ?? ""
        
        CareerFeeds.updateAthleteExtracurricular(careerId: self.careerId, title: titleTextField.text!, description: description, role: roleTextField.text!, extracurricularId: extracurricularId) { error in
            ...
            if (error == nil)
            {
                print("Update Extracurricular Success")
            ...
            {
                print("Update Extracurricular Failed")
    ...
    {
        MiscHelper.showAlert(in: self, withActionNames: ["Delete", "Cancel"], title: "Delete Extracurricular?", message: "Are you sure you want to delete this extracurricular from your profile?", lastItemCancelType: false) { tag in
            
            if (tag == 0)
            {
                print("Delete")
                let extracurricularId = self.dataObj["careerExtracurricularId"] as? String ?? ""
                
                CareerFeeds.deleteAthleteExtracurricular(careerId: self.careerId, extracurricularId: extracurricularId) { error in
                    
                    if (error == nil)
                    {
                        print("Delete Extracurricular Success")
                        
                        OverlayView.showTwoLinePopupOverlay(withMessage: "Extracurricular Deleted", boldText: "", withDismissHandler: {
                    ...
                    {
                        print("Delete Extracurricular Failed")
    ...
    @objc private func checkValidFields()
    {
        ...
        if (titleTextField.text!.count > 0) //&& (roleTextField.text!.count > 0))
        {
    ...
    {
        let title = dataObj["activity"] as? String ?? ""
