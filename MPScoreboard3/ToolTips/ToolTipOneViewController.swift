//
//  ToolTipOneViewController.swift
//  MPScoreboard3
//
//  Created by David Smith on 5/13/22.
//

import UIKit

protocol ToolTipOneDelegate: AnyObject
{
    func toolTipOneClosed()
}

class ToolTipOneViewController: UIViewController
{
    weak var delegate: ToolTipOneDelegate?
    ...
    @IBAction func closeButtonTouched()
    {
        ...
        { (finished) in
            ...
            kUserDefaults.setValue(NSNumber.init(booleanLiteral: true), forKey: kToolTipOneShownKey)
            
            self.delegate?.toolTipOneClosed()
            self.dismiss(animated: false)
        }
    }
    
    @IBAction func closeAllButtonTouched()
    {
        UIView.animate(withDuration: 0.33, animations: {
        ...
        { (finished) in
            ...
            kUserDefaults.setValue(NSNumber.init(booleanLiteral: true), forKey: kToolTipSixShownKey)
            kUserDefaults.setValue(NSNumber.init(booleanLiteral: true), forKey: kToolTipSevenShownKey)
            kUserDefaults.setValue(NSNumber.init(booleanLiteral: true), forKey: kToolTipEightShownKey)
    ...
    override func viewDidLoad()
    {
        ...
        let holeFrame = CGRect(x: 4.0, y: CGFloat(kStatusBarHeight + SharedData.topNotchHeight + 94), width: CGFloat(kDeviceWidth - 8), height: 44.0)
        self.addRoundRectBackgroundHole(frame: holeFrame)
        
        containerView.frame = CGRect(x: Int((kDeviceWidth - 302) / 2.0), y: kStatusBarHeight + SharedData.topNotchHeight + 136, width: 302, height: 281)
