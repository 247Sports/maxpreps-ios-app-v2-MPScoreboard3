//
//  NewAthleteProfileViewController.swift
//  MPScoreboard3
//
//  Created by David Smith on 6/8/23.
//

import UIKit
import AVFoundation

class NewAthleteProfileViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
    ...
    var careerId = ""
    var hideFullCareerButton = false
    ...
    {
        let sportTeams = self.profileData["sportTeams"] as! Array<Dictionary<String,Any>>
        
        myTeamsVC = MyTeamsViewController(nibName: "MyTeamsViewController", bundle: nil)
        myTeamsVC.sportTeams = sportTeams
    ...
    private func showClaimedProfilesViewController()
    {
        ...
        claimedProfilesVC.userProfileType = "Athlete"
    ...
    @IBAction func careerButtonTouched()
    {
        // Get the school name and color from the first team
        let sportTeams = self.profileData["sportTeams"] as! Array<Dictionary<String,Any>>
        
        if (sportTeams.count == 0)
        {
            MiscHelper.showAlert(in: self, withActionNames: ["OK", "Support"], title: "We're Sorry", message: "There was an error with the career profile linked to this account. Please contact support to resolve the issue.", lastItemCancelType: false) { (tag) in
                
                if (tag == 1)
                {
                    self.contactButtonTouched()
                }
            }
            return
        }
        
        let sportTeam = sportTeams.first
        let teams = sportTeam!["teams"] as! Array<Dictionary<String,Any>>
        let team = teams.first
        let schoolId = team!["schoolId"] as? String ?? kEmptyGuid
        let schoolName = team!["schoolName"] as? String ?? "Unknown School"
        let schoolState = team!["schoolState"] as? String ?? ""
        let schoolCity = team!["schoolCity"] as? String ?? ""
        let schoolColor = team!["schoolColor1"] as? String ?? "808080"
        
        let firstName = self.profileData["firstName"] as? String ?? ""
        let lastName = self.profileData["lastName"] as? String ?? ""
        
        let selectedAthlete = Athlete(firstName: firstName, lastName: lastName, schoolName: schoolName, schoolState: schoolState, schoolCity: schoolCity, schoolId: schoolId, schoolColor: schoolColor, schoolMascotUrl: "", careerId: self.careerId, photoUrl: "")
        
        // Check to see if the athlete is already a favorite
        let isFavorite = MiscHelper.isAthleteMyFavoriteAthlete(careerId: self.careerId)
    ...
    @objc private func contactButtonTouched()
    {
        self.showWebViewController(urlString: kTechSupportUrl, title: "Support")
    }
    ...
    override func viewDidLoad()
    {
        ...
        fullCareerButton.isHidden = self.hideFullCareerButton
        
        self.loadUserImage()
        self.getAthleteUserProfile()
        
        // Tracking
        TrackingManager.trackState(featureName: "membership-home", trackingGuid: trackingGuid, cData: kEmptyTrackingContextData)
