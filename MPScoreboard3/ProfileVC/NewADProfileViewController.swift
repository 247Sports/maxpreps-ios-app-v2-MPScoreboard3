//
//  NewADProfileViewController.swift
//  MPScoreboard3
//
...
class NewADProfileViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate
{
    ...
    private var adSchoolSelectorVC: ADSchoolSelectorViewController!
    ...
    private var adTeams = [] as Array<Dictionary<String,Any>>
    private var adSchools = [] as Array<Dictionary<String,Any>>
    private var tableHasMyTeams = false
    private var tableHasAdminTeams = false
    private var trackingGuid = ""
    
    // MARK: - Show AD School Selector View Controller
    
    private func showADSchoolSelectorViewController()
    {
        adSchoolSelectorVC = ADSchoolSelectorViewController(nibName: "ADSchoolSelectorViewController", bundle: nil)
        adSchoolSelectorVC.adTeams = adTeams
        adSchoolSelectorVC.adSchools = adSchools
        self.navigationController?.pushViewController(adSchoolSelectorVC, animated: true)
    }
    ...
    private func showClaimedProfilesViewController()
    {
        ...
        claimedProfilesVC.userProfileType = "AD"
    ...
    // MARK: - Get AD User Profile
    
    private func getADUserProfile()
    {
        ...
        NewFeeds.getADUserProfile(schoolId: self.schoolId) { result, error in
            ...
            if (error == nil)
            {
                ...
                if (self.profileData.count > 0)
                {
                    ...
                    {
                        self.tableHasMyTeams = false
                    }
                    
                    let adminTeams = self.profileData["adminTeams"] as! Array<Dictionary<String,Any>>
                    if (adminTeams.count > 0)
                    {
                        self.tableHasAdminTeams = true
                        self.viewProfileButton.isHidden = false
                    }
                    else
                    {
                        self.tableHasAdminTeams = false
                        self.viewProfileButton.isHidden = true
                    }
                    
                    // Build the AD schools array. One entry per school
                    if (self.profileData["athleticDirectorTeams"] != nil)
                    {
                        var schools = [] as Array<String>
                        self.adTeams = self.profileData["athleticDirectorTeams"] as! Array<Dictionary<String,Any>>
                    
                        for adTeam in self.adTeams
                        {
                            let schoolId = adTeam["schoolId"] as! String
                            if (schools.contains(schoolId) == true)
                            {
                                continue
                            }
                            else
                            {
                                schools.append(schoolId)
                                self.adSchools.append(adTeam)
                            }
                        }
                    
                        print(String(self.adSchools.count))
                    }
                }
            }
            else
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int
    {
        if (section == 0)
        {
            if ((tableHasMyTeams == true) && (tableHasAdminTeams == true))
            {
                return 4
            }
            else if ((tableHasMyTeams == false) && (tableHasAdminTeams == true))
            {
                return 3
            }
            else if ((tableHasMyTeams == true) && (tableHasAdminTeams == false))
            {
                return 3
            }
            else
        ...
        if (indexPath.section == 0)
        {
            if ((tableHasMyTeams == true) && (tableHasAdminTeams == true))
            {
                if (indexPath.row == 0)
                {
                    ...
                    cell?.titleLabel.text = "Athletic Director"
                    cell?.iconImageView.image = UIImage(named: "NewADIcon")
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        if (indexPath.section == 0) // Profile and Teams section
        {
            if ((tableHasMyTeams == true) && (tableHasAdminTeams == true))
            {
                if (indexPath.row == 0)
                {
                    self.showADSchoolSelectorViewController()
                }
                else if (indexPath.row == 1)
                {
    ...
    override func viewDidAppear(_ animated: Bool)
    {
        ...
        if (adSchoolSelectorVC != nil)
        {
            adSchoolSelectorVC = nil
        }
