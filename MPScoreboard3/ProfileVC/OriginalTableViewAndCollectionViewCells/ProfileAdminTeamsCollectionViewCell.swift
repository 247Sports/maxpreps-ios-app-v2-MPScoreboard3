//
//  ProfileAdminTeamsCollectionViewCell.swift
//  MPScoreboard3
//
...
class ProfileAdminTeamsCollectionViewCell: UICollectionViewCell
{
    ...
    func loadData(data: Dictionary<String,Any>)
    {
        ...
        let roleName = data["adminRoleTitle"] as! String
        titleLabel.text = roleName.uppercased()
        ...
        var genderSportLevel = MiscHelper.genderSportShortLevelFrom(gender: gender, sport: sport, level: level)
        genderSportLevel = genderSportLevel.replacingOccurrences(of: ".", with: "")
        sportLabel.text = genderSportLevel.uppercased()
    ...
    func addShapeLayers(color: UIColor)
    {
        ...
        rearPath.addLine(to: CGPoint(x: 30, y: containerView.frame.size.height))
        ...
        frontPath.addLine(to: CGPoint(x: 26, y: containerView.frame.size.height))
